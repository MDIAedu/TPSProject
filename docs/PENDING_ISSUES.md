...
## 보류된 의심
- 2026-07-24: 01-5에서 `APlayerCubeCharacter`에 조준 카메라/기본 사격 판정을 최소 구현으로 추가했고, 01-7에서 3단 사격 콤보 상태, 01-8에서 공격 몽타주와 Anim Notify 입력 허용 상태, 01-16에서 플레이어 HP 상태와 HP Widget 생성 연결도 같은 파일에 유지했다. 현재는 프로토타입 검증을 위해 유지하지만, 무기·피해량·탄약·피격 반응·플레이어 상태·UI 연결처럼 전투 범위가 더 커지면 별도 컴포넌트, 무기 클래스, PlayerController 또는 HUD 클래스로 분리할지 재검토한다.
