...
- 2026-08-12: 01-16에서 `AComfyUIWorkflowRequestActor`를 기존 `TPSProject` 런타임 모듈에 유지했다. 현재는 로컬 ComfyUI 요청 검증을 위한 최소 Actor라 유지하지만, 에디터 도구 UI, 에셋 임포트, workflow 관리 기능이 늘어나면 별도 Editor 모듈이나 에디터 유틸리티 구조로 분리할지 재검토한다.
